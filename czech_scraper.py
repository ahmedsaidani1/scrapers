"""
Czech Solar Scraper
Website: https://shop.czech.solar
"""
import sys
import re
...
class CzechScraper(BaseScraper):
    """
    Scraper for Czech Solar equipment website.
    """
    ...
    def __init__(self):
        ...
        self.base_url = self.config.get("base_url", "https://shop.czech.solar")
    ...
    def get_product_urls(self) -> List[str]:
        ...
        if response:
            ...
            for loc in locs:
                url = loc.text.strip()
                # Filter for product pages (Czech Solar uses /en/category/product-name/)
                if any(keyword in url.lower() for keyword in [
                    ...
                    if not url.endswith('/') or url.count('/') > 5:
    ...
    def _scrape_category_pages(self) -> List[str]:
        ...
        # Common category URLs for Czech Solar
        category_urls = [
            f"{self.base_url}/en/solar-modules/",
            f"{self.base_url}/en/inverters/",
            f"{self.base_url}/en/batteries/",
            f"{self.base_url}/en/complete-sets/",
        ]
        ...
        for category_url in category_urls:
            ...
            if response:
                ...
                for link in all_links:
                    ...
                    if any(keyword in href.lower() for keyword in ['/solar-modules/', '/inverters/', '/batteries/', '/complete-sets/']):
                        # Make absolute URL
                        if href.startswith('/'):
                        ...
                        if href not in product_urls and (not href.endswith('/') or href.count('/') > 5):
