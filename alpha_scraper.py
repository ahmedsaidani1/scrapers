"""
Alpha Solar Scraper
Website: https://www.alpha-solar.info
"""
import sys
import re
...
class AlphaScraper(BaseScraper):
    """
    Scraper for Alpha Solar equipment website.
    """
    ...
    def __init__(self):
        ...
        self.base_url = self.config.get("base_url", "https://www.alpha-solar.info")
        
        # Disable SSL warnings for this site (certificate issues)
        import urllib3
        urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
        
        # Login if required
        if self.config.get("requires_login"):
            self._login()
    ...
    def _login(self):
        ...
        try:
            ...
            self.logger.info("Setting up HTTP Basic Authentication")
            
            # Set up HTTP Basic Auth for the session
            ...
            self.logger.error(f"Authentication setup failed: {e}", exc_info=True)
    ...
    def get_product_urls(self) -> List[str]:
        ...
        response = self.make_request(sitemap_url, verify=False)  # Disable SSL verification
        ...
        if response:
            ...
            for loc in locs:
                url = loc.text.strip()
                # Filter for product pages (Alpha uses specific patterns)
    ...
    def _scrape_category_pages(self) -> List[str]:
        ...
        # Common category URLs for Alpha Solar
        category_urls = [
            f"{self.base_url}/Balkonkraftwerke/",
            f"{self.base_url}/Photovoltaik-Anlagen/",
            f"{self.base_url}/Wechselrichter/",
            f"{self.base_url}/Solarmodule/",
            f"{self.base_url}/Batteriespeicher/",
        ]
        ...
        for category_url in category_urls:
            ...
            if response:
                ...
                for link in all_links:
                    ...
                    if any(keyword in href.lower() for keyword in ['balkonkraftwerk', 'photovoltaik', 'wechselrichter', 'solarmodul', 'batteriespeicher']):
                        # Make absolute URL
                        if href.startswith('/'):
                        ...
                        if href not in product_urls and not href.endswith('/'):
