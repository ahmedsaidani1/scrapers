"""
Priwatt Scraper - With Login
Website: https://priwatt.de
"""
import sys
import re
...
class PriwattScraper(BaseScraper):
    """
    Scraper for Priwatt solar equipment website.
    ...
    def __init__(self):
        ...
        self.base_url = self.config.get("base_url", "https://priwatt.de")
    ...
    def get_product_urls(self) -> List[str]:
        ...
        if response:
            ...
            for loc in locs:
                url = loc.text.strip()
                # Filter for product pages (Priwatt uses /balkonkraftwerk-* and SW codes)
    ...
    def _scrape_category_pages(self) -> List[str]:
        ...
        # Common category URLs for Priwatt
        category_urls = [
            f"{self.base_url}/balkonkraftwerk-800-watt/",
            f"{self.base_url}/balkonkraftwerk-speicher/",
            f"{self.base_url}/balkonkraftwerk-mit-speicher/",
            f"{self.base_url}/sales/balkonkraftwerk-2000-watt/",
        ]
        ...
        for category_url in category_urls:
            ...
            if response:
                ...
                for link in all_links:
                    ...
                    if any(keyword in href.lower() for keyword in ['balkonkraftwerk', '/sw']):
                        # Make absolute URL
                        if href.startswith('/'):
