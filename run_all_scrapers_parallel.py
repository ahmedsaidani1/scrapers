...
Executes all 9 working scrapers simultaneously for maximum efficiency
"""
...
def main():
    """Main execution - runs all scrapers in parallel"""
    ...
    # Use number of CPUs available, but cap at number of scrapers
    num_processes = min(multiprocessing.cpu_count(), len(scrapers))
    
    print(f"\nUsing {num_processes} parallel processes")
    print(f"Scraping {len(scrapers)} websites...\n")
    
    # Run scrapers in parallel
    with multiprocessing.Pool(processes=num_processes) as pool:
        results = pool.starmap(run_scraper, scrapers)
